...
async def _select_api(user_input: str) -> Optional[dict]:
    """用 LLM 分析用户输入，选择最合适的 API"""
    endpoints_desc = "\n".join(
        f"- {e['path']}: {e['desc']}"
        for e in API_ENDPOINTS
    )

    prompt = f"""根据用户的问题，从以下 API 中选择最合适的一个。
只返回 API 路径，不要其他文字。
...
async def _generate_answer(user_input: str, api_path: str, api_data: dict) -> tuple:
    ...
    if list_data and isinstance(list_data, list) and len(list_data) > 0:
        first = list_data[0]
        if isinstance(first, dict) and ("resKey" in first or "day" in first):
            keys = [d.get("resKey") or d.get("day", "") for d in list_data]
            vals = [float(d.get("resValue") or d.get("totalRevenue", 0)) for d in list_data]
            label = "值"
